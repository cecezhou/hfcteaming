...
skill_nums = myargs["-skill_nums"].split(",")
skill_nums = [-int(x) for x in skill_nums]
...
W_s = N
K_s = int(np.floor(N/Q))
W_reg = W_s - W_slack
Q_reg = int(np.floor(W_reg/K_s))
Q_slack = int(np.floor(W_slack/K_s))
# assert skill number for each team is equal to the bounds
assert(len(skill_nums) == M)
assert(sum(skill_nums) == - K_s * p)
...
my_rhs = [0] * (2 * K_s * M ) + [p]* K_s + [1] * W_s + [-Q_reg] * K_s \
        + [Q_reg + 1] * K_s + skill_nums + [-Q_slack] * K_s + [Q_slack + 1] * K_s
my_sense = "L" * (2 * K_s * M) + "L" *  (K_s) + "E" * (W_s) + "L" * (K_s * 2) + "E" * (M) + "L" * (K_s * 2)
...
if s ==1:
    ...
    df_team_assigns.to_csv(str(filename) + "NewSpecAssignments" + str(N) + ".csv")
